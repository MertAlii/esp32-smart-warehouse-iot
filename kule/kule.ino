...
// YENİ DURUM: TEYIT_BEKLENIYOR
enum KuleDurumu { BEKLEME, URUN_ALGILANDI, KOL_BEKLENIYOR, TEYIT_BEKLENIYOR, ARABA_BEKLENIYOR, HATA };
...
void sendBantKomut(String komut); void sendKolKomut(String komut, String hedefKutu); void sendArabaKomut(String komut, String hedef);
...
void loop() {
  ...
  if (digitalRead(PIN_BUTON) == LOW) {
    if (mevcutDurum == HATA) {
      mevcutDurum = BEKLEME; addLog("Manuel Sifirlama!"); digitalWrite(PIN_BUZZER, LOW);
      for(int i = 0; i < 3; i++) {
        digitalWrite(PIN_LED_KIRMIZI, HIGH); digitalWrite(PIN_LED_SARI, HIGH); digitalWrite(PIN_LED_YESIL, HIGH); delay(200);
  ...
  if (currentMillis - lastDisplayUpdate > 3000) { currentPage = (currentPage + 1) % 6; lastDisplayUpdate = currentMillis; }
  ekranGuncelle();
...
void reconnectMQTT() {
  while (!mqtt.connected()) {
    if (mqtt.connect(client_id)) {
      if (ilkBaglanti) { baglantiZamani = millis(); ilkBaglanti = false; }
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strTopic == "akillidepo/ayar/sifreleme") {
    sifrelemeAktif = (msg == "1"); addLog(sifrelemeAktif ? "Sifreleme: ACIK" : "Sifreleme: KAPALI"); return; 
  }
  ...
  if (strTopic.endsWith("/heartbeat")) {
    ...
    if (strTopic.indexOf("bant") != -1) hbBant = nowMs; else if (strTopic.indexOf("kol") != -1) hbKol = nowMs;
    else if (strTopic.indexOf("araba") != -1) hbAraba = nowMs; return;
  }

  // --- İNATÇI VURUŞ (TEYİT) MANTIĞI BURADA İŞLİYOR ---
  if (strTopic == TOPIC_BANT_URUN) {
    String renk = doc["renk"];
    
    // Normal Ürün Okuma Durumu
    if (mevcutDurum == BEKLEME) {
      sonUrunRenk = renk; islenenUrunSayisi++; addLog("Urun: " + renk);
      mevcutDurum = URUN_ALGILANDI; kararVer(renk);
    }
    // Banttan gelen "Kontrol" raporu durumu
  ...
  // Kol vurduktan sonra Bant'tan Teyit İste!
    ...
    if (doc["tip"] == "teslim" && mevcutDurum == ARABA_BEKLENIYOR) {
      addLog("Araba: Teslim tmm"); mevcutDurum = BEKLEME; sendBantKomut("baslat"); 
    }
  }
}
...
void kararVer(String renk) {
  ...
  if (renk == "kirmizi" || renk == "sari") {
    if (!kolOnline) { addLog("HATA: Kol Offline!"); mevcutDurum = HATA; return; }
    ...
    if (!arabaOnline) { addLog("HATA: Araba Offline!"); mevcutDurum = HATA; return; }
    arabaUrunSayaci++; addLog("Arabaya: " + String(arabaUrunSayaci) + "/5");
    if (arabaUrunSayaci >= 5) {
      sonUrunHedef = "teslimat_noktasi"; sonUrunItildi = false; mevcutDurum = ARABA_BEKLENIYOR; 
      stateTimer = millis(); sendBantKomut("dur"); sendArabaKomut("git", sonUrunHedef); arabaUrunSayaci = 0; 
...
void durumKontrol() {
  ...
  // Teyit için banttan 5 saniye içinde cevap gelmezse hata ver
  if (mevcutDurum == TEYIT_BEKLENIYOR && (nowMs - stateTimer > 5000)) { mevcutDurum = HATA; stateTimer = nowMs; addLog("HATA: Teyit Timeout"); }
  if (mevcutDurum == ARABA_BEKLENIYOR && (nowMs - stateTimer > ARABA_TIMEOUT_MS)) { mevcutDurum = HATA; stateTimer = nowMs; addLog("HATA: Araba Timeout"); }
}
...
void ledVeBuzzerGuncelle() {
  ...
  if (mevcutDurum == HATA) { if ((nowMs / 250) % 2 == 0) { digitalWrite(PIN_LED_KIRMIZI, HIGH); digitalWrite(PIN_BUZZER, HIGH); } } 
...
void ekranGuncelle() {
  ...
  unsigned long nowMs = millis();
  String sBant = (nowMs - hbBant < HB_TIMEOUT) ? "ON" : "OFF"; String sKol = (nowMs - hbKol < HB_TIMEOUT) ? "ON" : "OFF"; String sAraba = (nowMs - hbAraba < HB_TIMEOUT) ? "ON" : "OFF";

  switch (currentPage) {
    case 0:
      display.println("1/6: GENEL DURUM"); display.drawLine(0, 10, 128, 10, SH110X_WHITE); display.setCursor(0, 15);
      display.println("Bant:  " + sBant); display.println("Kol:   " + sKol); display.println("Araba: " + sAraba); display.print("Sistem: ");
      if(mevcutDurum == BEKLEME) display.println("BEKLEME");
      else if(mevcutDurum == HATA) display.println("HATA!");
      else if(mevcutDurum == TEYIT_BEKLENIYOR) display.println("TEYIT EDIOR.."); // Yeni Ekran Yazısı
    ...
    case 2: 
      ...
      break;
    case 3: 
      display.println("4/6: ARABA DURUMU"); display.drawLine(0, 10, 128, 10, SH110X_WHITE); display.setCursor(0, 15);
      display.println("Komut: " + arabaSonKomut); display.println("Hedef: " + arabaSonHedef); display.print("Kargo: "); display.println(String(arabaUrunSayaci) + " / 5 Urun"); 
      break;
    case 4: 
      display.println("5/6: SISTEM LOG"); display.drawLine(0, 10, 128, 10, SH110X_WHITE); display.setCursor(0, 15);
    ...
    case 5:
      ...
      display.println("- Mert Ali Alkan"); display.println("- Umut Turker"); display.println("- Berk Talha Aslan");
      break;
  }
  display.display();
}

void addLog(String msj) { sysLogs[2] = sysLogs[1]; sysLogs[1] = sysLogs[0]; sysLogs[0] = msj; Serial.println("LOG: " + msj); }
...
void sendKolKomut(String komut, String hedefKutu) {
  StaticJsonDocument<256> doc; doc["komut"] = komut; doc["hedef_kutu"] = hedefKutu; doc["timestamp"] = millis(); publishMessage(TOPIC_KOL_KOMUT, doc);
}
void sendArabaKomut(String komut, String hedef) {
  StaticJsonDocument<256> doc; doc["komut"] = komut; doc["hedef"] = hedef; doc["adim_ms"] = 4000; arabaSonKomut = komut; arabaSonHedef = hedef; publishMessage(TOPIC_ARABA_KOMUT, doc);
}