...
void setup() {
  ...
  pinMode(PIN_LED_G, OUTPUT); pinMode(PIN_LED_B, OUTPUT);
  ledYak(0, 0, 0); 
  ...
  doc["durum"] = "online";
  doc["mesaj"] = "Bant hazir ve donuyor."; doc["timestamp"] = millis();
...
void ledYak(int r, int g, int b) {
  digitalWrite(PIN_LED_R, r); digitalWrite(PIN_LED_G, g);
  ...
  if (strTopic == TOPIC_BANT_KOMUT) {
    ...
    // YENİ ÖZELLİK: TEYİT KONTROLÜ
      ...
      if (suAnBosluk) {
        ...
        long g = renkFrekansOku(HIGH, HIGH); long b = renkFrekansOku(LOW, HIGH);
...
void renkKontrolVeGonder() {
  ...
  if (boslukBekleniyor) {
    if (suAnBosluk) {
      boslukBekleniyor = false;
      yeniUrunBekleniyor = true; ledYak(0, 0, 0); 
  ...
  if (!suAnBosluk && yeniUrunBekleniyor) {
    long r = renkFrekansOku(LOW, LOW); long g = renkFrekansOku(HIGH, HIGH);
